...
1. **유저 스토리 기반 검증** — Phase 2의 유저 스토리(`docs/02-planning/user-stories.md`)의 Acceptance Criteria를 하나씩 확인
   - Given-When-Then으로 정의된 각 AC가 실제로 동작하는지 테스트
   - 모든 Must Have 유저 스토리의 AC가 통과해야 릴리즈 가능

2. **KPI 측정 기반 검증** — Phase 2의 KPI 정의서(`docs/kpi-definitions.md`)에 정의된 기술 KPI 확인
...
### Step 8: 산출물 생성
- **`docs/07-qa/test-strategy.md`** — 테스트 전략 문서
- **`docs/07-qa/release-checklist.md`** — 릴리즈 체크리스트 (실제 체크용)
...
- Phase 2 유저 스토리(`docs/02-planning/user-stories.md`)와 KPI 정의서(`docs/kpi-definitions.md`)를 참조하여 인수 테스트를 수행한다
