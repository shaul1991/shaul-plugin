...
- **사내 자산 3계층 분류 (v0.8.0)** — `00-setup` Step 8 에서 ALM 산출물을 *공유 자산*(`docs/<name>/` 실체 + `.claude/<name>` 심볼릭 링크) / *운영 자산*(`.claude/` + `.gitignore` negate) / *로컬 전용*(`.claude/local/`, `.claude/settings.local.json`) 3 계층으로 분류 권유. 공유 자산은 단일 진실 + symlink 로 sync drift 위험 0. 자동 승격은 *하지 않으며* 사용자 명시 결정 시에만 진행. v0.7.x 이전의 `.claude/` 전체 차단 패턴은 그대로 유지 가능 (하위 호환).
