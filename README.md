...
- **시크릿 파일 가드 (v0.7.0)** — `Read`/`Edit`/`Write`/`Bash` 가 `.env`, `.env.*` 같은 시크릿 파일을 만지려 시도하면 *무조건* 차단(또는 사용자 확인 프롬프트). 어느 step·skill·에이전트에서도 동일 적용. 사용자가 `.claude/secret-guard.json` 한 파일만 편집해 정책을 추가·삭제 가능. 일시 해제는 `CLAUDE_PLUGIN_SECRET_GUARD=off`.
